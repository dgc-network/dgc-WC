...
if ( ! function_exists( 'wp_travel_create_new_customer' ) ) {
	...
	function wp_travel_create_new_customer( $email, $username = '', $password = '' ) {
		...
		if ( empty( $email ) || ! is_email( $email ) ) {
			return new WP_Error( 'registration-error-invalid-email', __( 'Please provide a valid email address.', 'text-domain' ) );
		}

		if ( email_exists( $email ) ) {
			return new WP_Error( 'registration-error-email-exists', apply_filters( 'wp_travel_registration_error_email_exists', __( 'An account is already registered with your email address. Please log in.', 'text-domain' ), $email ) );
		}
		...
		if ( 'no' === $generate_username_from_email || ! empty( $username ) ) {
			...
			if ( empty( $username ) || ! validate_username( $username ) ) {
				return new WP_Error( 'registration-error-invalid-username', __( 'Please enter a valid account username.', 'text-domain' ) );
			}

			if ( username_exists( $username ) ) {
				return new WP_Error( 'registration-error-username-exists', __( 'An account is already registered with that username. Please choose another.', 'text-domain' ) );
			}
		} else {
		...
		if ( 'yes' === $generate_user_password && empty( $password ) ) {
			...
			return new WP_Error( 'registration-error-missing-password', __( 'Please enter an account password.', 'text-domain' ) );
		} else {
		...
		if ( is_wp_error( $customer_id ) ) {
			return new WP_Error( 'registration-error', '<strong>' . __( 'Error:', 'text-domain' ) . '</strong> ' . __( 'Couldn&#8217;t register you&hellip; please contact us if you continue to have problems.', 'text-domain' ) );
		}
